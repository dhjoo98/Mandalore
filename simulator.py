...
#idle PE stems from these occasions:
    #When one of the input(scalar to each PE) is zero; For each pruning method, this is specifically,
        #Sanger: pruned attention score x value matrix 
        #Wanda : pruned FF weight x hidden_state
    
#number of cycle can be reduced with: 
    #Sanger: when an element of attention score is zero, vec-vec multiplication is skipped
    #Wanda: None (exclude packing) 

#Rather than Spatten, move onto Packing. 
    #Sanger: 
    #Wanda: N:M sparsity is Tensor Core specific, no need to explore this for 1-D array

class Mandalore:
    def __init__(self, num_PEs = 64 ):#, method):
    ...
    def process(self, packed_network):
        ...
        for metadata, (x, y, z) in packed_network:
            if metadata[0].startswith("CustomMatmul") and metadata[1] == torch.Size([12, 128, 64]): #QK 
                ...
                ## num_of_reduced_cycle = number of tiles x number of zeros (does not affect the number of idle PE bubble )
        ...
        print("PE Utilization:", self.multiplier.cycle_taken * self.num_PEs - self.multiplier.idle_PE_bubble / self.multiplier.cycle_taken * self.num_PEs )    
        ...
        for i in range(0, block_M):
            for j in range(0, block_K):
                    for k in range(0, block_N):
                        ...
                        #possible bottleneck due to read/write? 
...
#mandalore = Mandalore('Sanger')
#mandalore = Mandalore('Wanda')
mandalore = Mandalore()
